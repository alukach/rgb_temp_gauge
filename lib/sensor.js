...
module.exports = {
    ...
    getReading: () => {
        ...
        if (this.phant.publicKey && this.phant.privateKey) {
            this.publishGoogleSheet(readout);
        }
    ...
    publishGoogleSheet: (readout) => {
        this.sheet.addRow(1, readout, (err) => {
            if (err) throw new Error(err);
        });
    },
    publishPhant: (readout) => {
        var data = querystring.stringify(readout);
        var url = `${this.phant.inputUrl}?private_key=${this.phant.privateKey}&${data}`
        https.get(url, (res) => {
